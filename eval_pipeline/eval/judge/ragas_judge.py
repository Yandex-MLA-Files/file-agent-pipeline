...
def _localize_prompts_to_russian(
    ...
    nli_instruction = (
        ...
        "Если контекст перечисляет составные части через уточняющие обороты "
        "(«X, который делает A, и Y, который делает B») в ответ на вопрос "
        "о составе, это подтверждает утверждение о составе («состоит из X и "
        "Y»), даже если слово «состоит» не встречается дословно."
    )
    ...
    nli_examples = [
        ...
        (
            nli_in_1.model_copy(
                update={
                    "context": (
                        "Ядро (kernel) операционной системы управляет аппаратными "
                        "ресурсами, а пользовательское пространство (userspace) "
                        "содержит прикладные программы."
                    ),
                    "statements": [
                        "Операционная система состоит из ядра (kernel) и "
                ...
                update={
                    "statements": [
                        nli_out_1.statements[0].model_copy(
                            update={
                                "statement": (
                                    "Операционная система состоит из ядра (kernel) "
                                    "и пользовательского пространства (userspace)."
                                ),
                                "reason": (
                                    "Контекст описывает ядро и пользовательское "
                                    "пространство как части операционной системы, "
                                    "перечисляя их функции по отдельности — слова "
                                    "«состоит» в контексте нет, но само "
                                    "перечисление двух частей уже подтверждает "
                                    "утверждение о составе."
                                ),
                                "verdict": 1,
                            }
                        ),
    ...
    answer_correctness.claim_decomposition_prompt.instruction = (
        ...
        "Если предложение перечисляет составные части через уточняющие обороты "
        "(«X, который делает A, и Y, который делает B»), обязательно выдели "
        "отдельным утверждением сам факт состава («состоит из X и Y») в "
        "дополнение к утверждениям про A и B — не заменяй его ими.\n"
    ...
    answer_correctness.claim_decomposition_prompt.examples = [
        ...
        (
            cd_in_1.model_copy(
                update={
                    "response": (
                        "Операционная система состоит из двух основных частей: "
                        "ядра (kernel), которое управляет аппаратными ресурсами, и "
                        "пользовательского пространства (userspace), которое "
                        "содержит прикладные программы."
                    )
                }
            ),
            cd_out_1.model_copy(
                update={
                    "claims": [
                        "Операционная система состоит из ядра (kernel) и "
                        "пользовательского пространства (userspace).",
                        "Ядро (kernel) управляет аппаратными ресурсами.",
                        "Пользовательское пространство (userspace) содержит прикладные программы.",
                    ]
                }
            ),
        ),
