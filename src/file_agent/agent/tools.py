...
them and the UI can list them as sources). Tools never call the answering LLM
themselves; ``inspect_image`` is the one that asks the vision model instead.
...
from file_agent.agent.tables import (
    MARKDOWN_SEPARATOR,
    TableView,
    document_tables,
    parse_header_row,
...
# inspect_image: answers longer than this are cut - the VLM already keeps them short.
MAX_IMAGE_ANSWER_CHARS = 3000
_DEFAULT_IMAGE_PROMPT = (
    "Опиши подробно, что изображено: подписи, оси, легенды, значения и весь "
    "видимый текст. Числа передавай в точности как на изображении."
)


def _is_table_block(block: Block) -> bool:
    return block.type == "xlsx_sheet" or block.block_type == BlockType.TABLE
...
def build_default_tools(
    ...
    if any(_image_blocks(document) for document in documents):
        # Registered only when some figure kept its pixels, so the model never
        # plans around a tool that would immediately refuse.
        factory = vlm_factory if vlm_factory is not None else _default_vlm_factory()
        tools.append(
            Tool(
                name="inspect_image",
                description=(
                    "Look at a figure or image inside a document with the vision "
                    "model and ask it a question (what a chart shows, a value on an "
                    "axis, text inside a diagram). Call it without 'image' first to "
                    "list a document's images, then with 'image' and a specific "
                    "'question'. Use it when the text around a figure does not "
                    "answer the question."
                ),
                parameters={
                    "file_name": "string, required - document file name",
                    "image": "integer, optional - image number from the listing",
                    "question": "string, optional - what to find out about the image",
                },
                run=lambda **kwargs: _inspect_image(documents, registry, factory, **kwargs),
            )
        )
    return tools
...
def _search_documents(
    ...
    **extra: Any,
    ...
    # The retriever reranks, diversifies and dedups internally; the pool only
    # needs enough slack for the client-side fusion of the formulations.
    pool = min(MAX_SEARCH_POOL, max(MIN_SEARCH_POOL, top_k + 3))

    fused: dict[tuple[str, str], float] = {}
    first_hit: dict[tuple[str, str], SearchResult] = {}
    for formulation in formulations:
        if document is not None:
            hits = retriever.search(query=formulation, top_k=pool, source_file=document.file_name)
        else:
            hits = retriever.search(query=formulation, top_k=pool)
    ...
    if document is not None:
        # Safety net for retrievers that ignore ``source_file``.
...
def _snippets(
    ...
    text = block.text
    if block.type == "xlsx_sheet" or block.block_type == BlockType.TABLE:
...
def _block_neighbourhood(blocks: list[Block], index: int) -> str:
    ...
    for block in reversed(blocks[max(0, index - 12) : index]):
        ...
        if not rendered or _is_table_block(block):
            continue
    ...
    for block in blocks[index + 1 : index + 13]:
        if block.block_type == BlockType.HEADING or _is_table_block(block):
            break
        rendered = block.to_markdown().strip()
        if not rendered:
            continue
...
def _row_snippets(text: str, matches: list[re.Match[str]]) -> list[str]:
    ...
    for match in matches:
        ...
        if MARKDOWN_SEPARATOR.match(lines[row]):
            continue
...
def _document_overview(document: Document) -> str:
    ...
    sheets = [
        ...
        if _is_table_block(block) and block.metadata.get("sheet_name")
    ]
    if sheets:
        lines.append("Sheets:")
        for block in sheets:
            rows = block.metadata.get("row_count") or block.metadata.get("max_row")
            columns = block.metadata.get("max_column")
            if rows and columns:
                size = f"{rows} rows x {columns} columns"
            elif rows:
                size = f"{rows} rows"
            else:
                size = ""
            header = block.metadata.get("table_header")
            if header:
                first_line = " | ".join(str(cell) for cell in header)[:200]
            else:
                first_line = block.text.split("\n", 1)[0].replace("\t", " | ")[:200]
            lines.append(f"  - {block.metadata['sheet_name']}: {size}; columns: {first_line}")
    elif tables:
        lines.append(f"Tables: {len(tables)} (query_table can compute over them)")
    images = _image_blocks(document)
    if images:
        lines.append(f"Images/figures: {len(images)} (inspect_image can look at them)")
...
def _extent(document: Document) -> str:
    ...
    sheets = {
        block.metadata.get("sheet_name")
        for block in document.blocks
        if block.metadata.get("sheet_name")
    }
    if sheets:
...
def _image_blocks(document: Document) -> list[Block]:
    """Figure blocks whose pixels survived parsing, in document order."""
    return [
        block
        for block in document.blocks
        if block.block_type in (BlockType.FIGURE, BlockType.IMAGE)
        and getattr(block, "image_bytes", None)
    ]
...
def _inspect_image(
    ...
    **extra: Any,
    ...
    _reject_unknown_arguments("inspect_image", extra)

    file_name = str(file_name).strip()
    if not file_name:
        raise ToolError("inspect_image requires the 'file_name' argument.")
    document = _find_document(documents, file_name)
    blocks = _image_blocks(document)
    if not blocks:
        raise ToolError(
            f"'{document.file_name}' has no stored images; its figures are already "
            "described in the text, use search_documents or read tools."
        )

    if image is None:
        listing = [f"{len(blocks)} image(s) in '{document.file_name}':"]
        for index, block in enumerate(blocks, start=1):
            where = f"page {block.page_number}" if block.page_number is not None else "no page"
            caption = " ".join((block.vlm_description or block.text or "").split())[:160]
            listing.append(f"  {index}. ({where}) {caption or 'no caption'}")
        listing.append("Call inspect_image again with 'image' (the number above) and a 'question'.")
        return ToolResult(output="\n".join(listing))

    index = _clamp_int(image, "image", 1, len(blocks), 0)
    if index == 0:
        raise ToolError(f"'image' must be a number between 1 and {len(blocks)}.")
    block = blocks[index - 1]

    client = vlm_factory()
    if client is None:
        raise ToolError(
            "No vision model is configured (VLM_BACKEND is off); rely on the "
            "figure descriptions already present in the text."
        )
    ...
    try:
        pixels = PILImage.open(io.BytesIO(block.image_bytes))
    except Exception as exc:  # noqa: BLE001 - any decode error reads the same
        raise ToolError(
            f"Image {index} of '{document.file_name}' cannot be decoded: {exc}"
        ) from exc

    question = " ".join(str(question or "").split())
    prompt = (
        f"Ответь на вопрос по изображению, точно передавая числа и подписи.\nВопрос: {question}"
        if question
        else _DEFAULT_IMAGE_PROMPT
    )
    answer = str(client.describe_image(pixels, prompt, max_tokens=800) or "").strip()
    if len(answer) > MAX_IMAGE_ANSWER_CHARS:
        answer = answer[:MAX_IMAGE_ANSWER_CHARS].rstrip() + " ..."
    if not answer:
        raise ToolError("The vision model returned an empty answer; try another question.")

    caption = " ".join((block.text or "").split())[:120]
    header = f"Image {index} of '{document.file_name}'"
    if caption:
        header += f" ({caption})"
    body = f"{header}\nQuestion: {question or 'describe'}\nVLM: {answer}"

    metadata: dict[str, Any] = {"image_index": index}
    if block.page_number is not None:
        metadata["page_number"] = block.page_number
    if block.metadata.get("slide_number") is not None:
        metadata["slide_number"] = block.metadata["slide_number"]
    passage = registry.add_text(
        body,
        document.file_name,
        metadata=metadata,
    ...
    logger.info("inspect_image(%r, image=%d) answered %d chars", file_name, index, len(answer))
    return ToolResult(
        output=passage.render(),
        sources=[passage.as_search_result()],
        passages=[passage],
...
def _default_vlm_factory() -> Callable[[], Any]:
    """Create the vision client once, on first use, from the environment."""
    cache: dict[str, Any] = {}
    ...
    def factory() -> Any:
        if "client" not in cache:
            ...
            cache["client"] = create_vlm_client()
        return cache["client"]
