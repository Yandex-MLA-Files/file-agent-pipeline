"""Original section-packing chunker, kept as the ``legacy`` strategy.

Selected with ``CHUNKING_STRATEGY=legacy`` (or ``chunk_document(strategy="legacy")``).
The default ``structured`` chunker in :mod:`file_agent.chunking` supersedes it
with heading-path breadcrumbs, per-block-type splitting and windowed parent
context; this copy is retained unchanged so results can be reproduced.
"""
...
from file_agent.chunking import (
    DEFAULT_MAX_CHARS,
    DEFAULT_OVERLAP,
    HEADER_REPEAT_MAX_RATIO,
...
def chunk_document_legacy(
    ...
    with tracer.start_as_current_span("file_agent.chunk_document_legacy") as span:
