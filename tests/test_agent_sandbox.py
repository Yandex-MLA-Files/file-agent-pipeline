...
def test_run_sandboxed_code_reports_missing_input_file(tmp_path):
    result = run_sandboxed_code(source_path=tmp_path / "missing.xlsx", code="print(1)")
...
def test_run_sandboxed_code_returns_captured_output(monkeypatch, tmp_path):
    ...
    result = run_sandboxed_code(source_path=source_path, code="print(42)")
    ...
    assert f"{source_path.resolve()}:/data/input.xlsx:ro" in " ".join(command)
...
def test_run_sandboxed_code_kills_container_on_timeout(monkeypatch, tmp_path):
    ...
    result = run_sandboxed_code(source_path=source_path, code="while True: pass", timeout_seconds=1)
...
def test_run_sandboxed_code_reports_missing_docker_cli(monkeypatch, tmp_path):
    ...
    result = run_sandboxed_code(source_path=source_path, code="print(1)")
...
def test_run_sandboxed_code_truncates_long_output(monkeypatch, tmp_path):
    ...
    result = run_sandboxed_code(
        source_path=source_path, code="print('x' * 100)", max_output_chars=10
    )
...
def test_run_sandboxed_code_surfaces_non_zero_exit_codes(monkeypatch, tmp_path, exit_code):
    ...
    result = run_sandboxed_code(source_path=source_path, code="raise ValueError()")
