...
from file_agent.agent.tables import document_tables
...
class FakeRetriever:
    def __init__(self, results: list[SearchResult] | None = None, by_query: dict | None = None):
        ...
        self.source_files: list[str | None] = []
    ...
    def search(self, query: str, top_k: int = 5, source_file: str | None = None):
        ...
        results = list(self.by_query.get(query, self.results))
        if source_file:
            results = [
                result
                for result in results
                if str(result.chunk.metadata.get("source_file", "")) == source_file
            ]
...
def test_search_documents_can_be_restricted_to_one_document():
    ...
    # The restriction rides down to the retriever instead of over-fetching.
    assert retriever.source_files[0] == "b.pdf"
    assert retriever.search_calls[0][1] == min(MAX_SEARCH_POOL, max(MIN_SEARCH_POOL, 5 + 3))
...
def test_search_documents_clamps_top_k_and_validates_arguments():
    ...
    assert retriever.search_calls == [
        ("q", min(MAX_SEARCH_POOL, max(MIN_SEARCH_POOL, MAX_SEARCH_TOP_K + 3)))
    ]
...
def test_list_documents_shows_structure_sheets_and_preview():
    ...
    assert "Inventory: 4 rows x 5 columns; columns: id | product | stock | price | category" in (
        result.output
    )
...
# ---------------------------------------------------------------------------
# structured spreadsheets (Markdown tables that remember their worksheet)
# ---------------------------------------------------------------------------


def make_structured_sheet_document() -> Document:
    """The shape the structured xlsx parser produces: Markdown per table region."""
    ratings = (
        "| player | FIDE | Chess.com |\n"
        "| --- | --- | --- |\n"
        "| Anna | 2410 | 2500 |\n"
        "| Boris | 1260 | 1300 |"
    )
    extras = "| player | puzzles |\n| --- | --- |\n| Anna | 900 |"
    return Document(
        ...
        blocks=[
            ...
            Block(
                ...
                metadata={
                    ...
                    "sheet_name": "Ratings",
                    "row_count": 2,
                    "table_header": ["player", "FIDE", "Chess.com"],
                },
                block_type=BlockType.TABLE,
                page_number=1,
            ),
            Block(
                ...
                text=extras,
                type="table",
                metadata={"source_file": "ratings.xlsx", "sheet_name": "Ratings"},
                block_type=BlockType.TABLE,
                page_number=1,
            ),
        ],
...
def test_document_tables_reads_markdown_sheet_tables_with_unique_names():
    views = document_tables(make_structured_sheet_document())

    assert [view.name for view in views] == ["Ratings", "Ratings #2"]
    assert all(view.origin == "sheet" for view in views)
    frame = views[0].frame
    assert list(frame.columns) == ["player", "FIDE", "Chess.com"]
    assert frame["FIDE"].dtype.kind in "if"
    assert frame.loc[frame["FIDE"] == 1260, "player"].tolist() == ["Boris"]


def test_query_table_selects_structured_sheet_by_name():
    tool = get_tool(
        build_default_tools(FakeRetriever(), [make_structured_sheet_document()]), "query_table"
    )

    result = tool.run(
        file_name="ratings.xlsx",
        sheet="Ratings",
        code="print(int(df['Chess.com'].max()))",
    )

    assert "2500" in result.output


def test_find_text_renders_markdown_table_hits_as_rows_with_header():
    tool = get_tool(
        build_default_tools(FakeRetriever(), [make_structured_sheet_document()]), "find_text"
    )

    result = tool.run(pattern="1260")

    assert "| player | FIDE | Chess.com |" in result.output
    assert "| Boris | 1260 | 1300 |" in result.output


def test_list_documents_summarises_structured_sheets_from_table_metadata():
    tool = get_tool(
        build_default_tools(FakeRetriever(), [make_structured_sheet_document()]), "list_documents"
    )

    output = tool.run().output

    assert "1 sheet(s)" in output
    assert "Ratings: 2 rows; columns: player | FIDE | Chess.com" in output
...
class FakeVLM:
    def __init__(self, answer: str = "На графике рост до 96 млрд."):
        self.answer = answer
...
def make_figure_document() -> Document:
    return Document(
        file_name="slides.pptx",
        file_type="pptx",
        blocks=[
            paragraph("intro", "About revenue."),
            Block(
                id="fig-1",
                text="Revenue chart 2024",
                type="figure",
                metadata={"source_file": "slides.pptx", "slide_number": 3},
...
def test_inspect_image_lists_images_then_answers_a_question():
    vlm = FakeVLM()
    tools = build_default_tools(FakeRetriever(), [make_figure_document()], vlm_factory=lambda: vlm)
    tool = get_tool(tools, "inspect_image")

    listing = tool.run(file_name="slides.pptx")
    assert "1 image(s)" in listing.output
    assert "page 3" in listing.output
    assert "Revenue chart 2024" in listing.output

    result = tool.run(file_name="slides.pptx", image=1, question="Какой рост?")
    assert "96 млрд" in result.output
    assert result.passages and result.passages[0].source_file == "slides.pptx"
    assert result.passages[0].metadata["image_index"] == 1
    assert "Какой рост?" in vlm.prompts[0]


def test_inspect_image_absent_without_stored_images_and_errors_without_vlm():
    tools = build_default_tools(FakeRetriever(), [make_document()])
    assert all(tool.name != "inspect_image" for tool in tools)

    tools = build_default_tools(FakeRetriever(), [make_figure_document()], vlm_factory=lambda: None)
    tool = get_tool(tools, "inspect_image")
    with pytest.raises(ToolError, match="vision model"):
        tool.run(file_name="slides.pptx", image=1, question="q")
